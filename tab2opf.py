...
import codecs
#fr = open(FILENAME,'rb')
fr = codecs.open(FILENAME, 'rb', encoding='utf-8')
...
for r in fr.readlines():
    ...
    if i % 10000 == 0:
        if to:
            ...
            """)
            to.close()
        to = codecs.open("%s%d.html" % (name, i / 10000), 'w', encoding='utf-8')
...
""")
    ...
    to.write("""      <idx:entry name="word" scriptable="yes">
      ...
      <mbp:pagebreak/>
""" % (
        yomi, #.replace(u'-', u'').replace(u'・', u''), kanji,
        dt, dtstrip, orth_alter_tag, dd))
    i += 1
    print dt
